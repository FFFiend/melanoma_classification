...
The entire training process (epoch #, batch sizes, etc) can be seen inside the .ipynb file, and the model architecture is located inside ```model.py``` and ```model.pt``` contains my trained model's weights. Feel free to look around.
